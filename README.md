...
```html
<div class="dynamic-text" data-texts="Hello World, Welcome to DanVerse, Loading something... Making some time..."></div>
```
...
- `pause`: Pause before deleting (ms).
- `startDelay`: Pause after deleting before typing again (ms).
- `humanity`: Organic variation (0 to 1).
- `random`: Shuffle list toggle (true/false).

### `engine.unregister(element)`
Stops the animation, clears timers, and removes the element from tracking. Essential for Single Page Applications (SPA) to prevent memory leaks.

## 📄 License
This project is licensed under the MIT License - see the [LICENSE](LICENSE) file for details.

---
Built with ❤️ by [Tu Nombre/GitHub] as part of the DanVerse Ecosystem.
